...
PortFunctionInit(void)
{
    ...
    GPIO_PORTN_DIR_R |= 0x03; //0x02;
	
		// Set the direction of PJ0 (SW1) as input by clearing the bit
    GPIO_PORTJ_AHB_DIR_R &= ~0x02; //0x01;
	
    // Enable PN1 for digital function.
    GPIO_PORTN_DEN_R |= 0x03; //0x02;
	
    // Enable PJ0 for digital function.
    GPIO_PORTJ_AHB_DEN_R |= 0x02; //0x01;
		
		//Enable pull-up on PJ0
		GPIO_PORTJ_AHB_PUR_R |= 0x02; //0x01; 
...
int main(void)
{
    ...
    while(1)
    {
        ...
        if((GPIO_PORTJ_AHB_DATA_R&0x02)==0x00) //SW1 is pressed
				{
						// Turn on the LED.
						GPIO_PORTN_DATA_R |= 0x03; //0x02;
				}
				else
				{
						// Turn off the LED.
