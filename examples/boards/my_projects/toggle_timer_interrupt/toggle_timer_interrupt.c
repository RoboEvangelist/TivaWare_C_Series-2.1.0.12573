...
//! interrupt to toggle the on-board LED D1.
//
//*****************************************************************************
...
{   
  ...
  TIMER0_TAILR_R = period-1;       // reload value
	NVIC_PRI4_R &= ~0xE000000; 	 // configure Timer0A interrupt priority as 0
...
Button_Interrupt_Init(void)
{
  NVIC_EN1_R |= 0x00080000;  		    // enable interrupt 51 in NVIC (GPIOJ)
	NVIC_PRI12_R &= ~0xE0200000; 		  // configure GPIOJ interrupt priority as 2
...
int main(void)
{	
		// clock running at 16MHz
		unsigned long period = 8000000*2; //reload value to Timer0A to generate one second delay
    ...
    GPIO_PORTN_DATA_R |= 0x00;
		...
		// give half second delay for me to see the count start on the LEDs
	  SysCtlDelay(8000000);
