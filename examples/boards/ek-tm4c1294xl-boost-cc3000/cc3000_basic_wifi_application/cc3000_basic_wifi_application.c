...
//*****************************************************************************
//
// Global to turn autonomous mode on/off
//
//*****************************************************************************
...
#define SYSCTL_RCGC0_R          (*((volatile uint32_t *)0x40038000))
...
CMD_sendData(int argc, char **argv)
{
    ...
    g_tSocketAddr.sa_data[4] = ui8IPBlock3;
		// last byte is the server's (the one receiving the data)
    ...
    else if(g_ui32SocketType == IPPROTO_TCP)
    {
        ...
        do
        {
            ...
            i32ReturnValue = recv(g_ui32Socket, g_pui8CC3000_Rx_Buffer,
					 ...
					 if(i32ReturnValue > 4)    // get into autonomous mode
					 {
							if(g_pui8CC3000_Rx_Buffer[0] == 'a' && g_pui8CC3000_Rx_Buffer[1] == 'u' &&
									g_pui8CC3000_Rx_Buffer[2] == 't' && g_pui8CC3000_Rx_Buffer[3] == 'o' &&
									g_pui8CC3000_Rx_Buffer[4] == '1')
							{	
								UARTprintf(" \n autonomous mode on...\n");
								autonomousMode = true;
							}
						...
						{
//							if(g_pui8CC3000_Rx_Buffer[0] == 'S' && g_pui8CC3000_Rx_Buffer[1] == 'T' &&
//									g_pui8CC3000_Rx_Buffer[2] == 'O' && g_pui8CC3000_Rx_Buffer[3] == 'P')
//							{
								UARTprintf(" stopping...\n");
								STOP();
//							}
						...
						else if(i32ReturnValue==3)
						{
							...
							else if(g_pui8CC3000_Rx_Buffer[0] == 'L' && g_pui8CC3000_Rx_Buffer[1] == 'F' &&
									g_pui8CC3000_Rx_Buffer[2] == '2')
							{
								UARTprintf("LF2 checked...\n");
								LForward2();
							}
							else if(g_pui8CC3000_Rx_Buffer[0] == 'L' && g_pui8CC3000_Rx_Buffer[1] == 'B' &&
									g_pui8CC3000_Rx_Buffer[2] == '1')
							{
							...
							else if(g_pui8CC3000_Rx_Buffer[0] == 'R' && g_pui8CC3000_Rx_Buffer[1] == 'F' &&
									g_pui8CC3000_Rx_Buffer[2] == '1')
							{
							...
							else if(g_pui8CC3000_Rx_Buffer[0] == 'R' && g_pui8CC3000_Rx_Buffer[1] == 'B' &&
									g_pui8CC3000_Rx_Buffer[2] == '1')
							{
						...
						{
							if(g_pui8CC3000_Rx_Buffer[0] == 'F' && g_pui8CC3000_Rx_Buffer[1] == '1') //SW1 is pressed//(count==0)	//
							{
								UARTprintf(" moving Forward F1...\n");
								Forward1();
							}
							//else if(strcmp(g_pui8CC3000_Rx_Buffer,"F2")==0)//(count==1)	//		//sw2 is pressed
							else if (g_pui8CC3000_Rx_Buffer[0] == 'F' && g_pui8CC3000_Rx_Buffer[1] == '2')
							{
								UARTprintf("    F2 checked\n");
								Forward2();
							}
							else if(g_pui8CC3000_Rx_Buffer[0] == 'B' && g_pui8CC3000_Rx_Buffer[1] == '1')//(c==B1)//(count==3)								//
							{
								UARTprintf("    B1 checked\n");
								Backward1();
							}
							else if(g_pui8CC3000_Rx_Buffer[0] == 'B' && g_pui8CC3000_Rx_Buffer[1] == '2')//(c==B2)//(count==4)		//		//sw2 is pressed
							{
...
/*            for(ui32x = 0; ui32x < i32ReturnValue; ui32x++, ui32Count++)
            ...
            //bRunOnce = false;
						...
						// if not autonomous mode, keep loop alive
						if(!autonomousMode)
						{
							if(bRunOnce)
							{
...
{
    uint32_t ui32Port = 5005;     // lets hard code the port number
    int8_t i8Check = 0;
    ...
    //
//    ui32Port = ustrtoul(port_number,0,10);      //convert port string to int
    g_tSocketAddr.sa_data[0] = (ui32Port & 0xFF00) >> 8;
    g_tSocketAddr.sa_data[1] = (ui32Port & 0x00FF) >> 0;
    ...
    memset (&g_tSocketAddr.sa_data[2], 0, 4);
		UARTprintf("    Address port %d\n", g_tSocketAddr.sa_data[2]);

    //
    // Low Level API call
    //
    i8Check = bind(g_ui32Socket, &g_tSocketAddr, sizeof(sockaddr));
		//listen(g_ui32Socket, 100);
	...
	
{
	...
	//char tcp_char[] = "socketclose TCP";
	
	//SW1 is pressed
	if(GPIO_PORTJ_AHB_RIS_R&0x01)
		//UARTprintf(" sw1 pressed turn on LED ...\n");
	{
		UARTprintf(" sw1 pressed turn on LED ...\n");
		//GPIOPinWrite(GPIO_PORTN_BASE, GPIO_PIN_1, 0x02);// turn on the LED
		//i32CommandStatus = CmdLineProcess(tcp_char);      // close a TCP socket
		UARTprintf(" stopping...\n");
		PWMPulseWidthSet(PWM0_BASE, PWM_OUT_2, 4000);			// 100% duty cycle PF 2
								PWMPulseWidthSet(PWM0_BASE, PWM_OUT_3, 4000);			// 100% duty cycle
								PWMPulseWidthSet(PWM0_BASE, PWM_OUT_4, 4000);			//100% duty cycle PF 1
								PWMPulseWidthSet(PWM0_BASE, PWM_OUT_5, 4000);			//100% duty cycle	PF 0
...
main(void)
{
    ...
    g_ui32BindFlag = SENTINEL_EMPTY;
    g_ui32SmartConfigFinished = 0;
		// commands to start the TCP socket
		char tcp_char[] = "socketopen TCP";
	  char bind_char[] = "bind 5005";
		char receive_data[] = "receivedata";
	  int32_t sample[CC3000_APP_BUFFER_SIZE +
                                            CC3000_RX_BUFFER_OVERHEAD_SIZE];
...
//				i32CommandStatus = CmdLineProcess(send_data);
				...
				// call the bind and socketopen commands
			  //UARTgets(g_cInput,sizeof(g_cInput));
        ...
        // It will be parsed and valid commands executed.
        // 
//        i32CommandStatus = CmdLineProcess(tcp_char);
//			  i32CommandStatus = CmdLineProcess(bind_char);
/*
        //
