...
EK-TM4C1294XL LaunchPad.  Run the application to download new firmware
and driver patches to the CC3000 processor.  Status is output via UART0
which is available via the virtual COM port provided by the ICDI debug
interface.
...
By default, the application is configured to expect the CC3000 BoosterPack
to be connected to the BoosterPack 2 connector on EK-TM4C1294XL. It may,
however, be rebuilt to support a CC3000 connected to the board's
BoosterPack 1 connector by replacing the label ``CC3000_USE_BOOSTERPACK2''
in the build environment with ``CC3000_USE_BOOSTERPACK1''.
