...
ScreenInit(void)
{
    ...
    GrStringDrawCentered(&g_sContext, "Basic WiFi App", -1,
    ...
    GrStringDrawCentered(&g_sContext, "Connect to", -1,
                         GrContextDpyWidthGet(&g_sContext) / 2, 33, 0);

    GrStringDrawCentered(&g_sContext, "terminal", -1,
                     GrContextDpyWidthGet(&g_sContext) / 2, 45, 0);

    GrStringDrawCentered(&g_sContext, "8-N-1 @ 115200", -1,
                         GrContextDpyWidthGet(&g_sContext) / 2, 57,0);
...
CMD_help(int argc, char **argv)
{
    ...
    {
        //
        // Show help information for a single command.
        //
        ...
        //
        // Wait for the UART to catch up before we send the next block of
        // characters.
        //
        UARTFlushTx(false);
    }
...
CMD_sendData (int argc, char **argv)
{
    ...
    //
    // Validate the message size, between 1 and 1460 bytes.
    //
...
CMD_bind (int argc, char **argv)
{
    ...
    {
        UARTprintf("    Invalid Port, must be between 0 and 65535.\n");
        return(CMDLINE_INVALID_ARG);
    }
    else if(g_ui32Socket == SENTINEL_EMPTY)
    {
        UARTprintf("    Socket not open, please run 'socketopen' first.\n");
        return(0);
    }
    else if( argc < 2 )
    {
    ...
    //
    // Tell the CC3000 to bind the socket.
    //
...
CMD_ipConfig (int argc, char **argv)
{
    ...
    {
        //
        // When using DHCP, set all variables = 0.
        //
        ui8NetMask[0] = 0;
        ui8NetMask[1] = 0;
        ui8NetMask[2] = 0;
    ...
    //
    // Tell CC3000 to initiate DHCP and acquire an IP address.
    //
...
CMD_mdnsadvertise (int argc, char **argv)
{
    ...
    if(1 == argc)
    {
        ...
        {
            i32Check = mdnsAdvertiser(1,g_pcdevice_name,sizeof(g_pcdevice_name));
        }
...
main(void)
{
    ...
    while(1)
    {
        ...
        {
            //
            // Approximately 1 milisecond delay.
            //
            ROM_SysCtlDelay(ROM_SysCtlClockGet() / 3000);
        }
