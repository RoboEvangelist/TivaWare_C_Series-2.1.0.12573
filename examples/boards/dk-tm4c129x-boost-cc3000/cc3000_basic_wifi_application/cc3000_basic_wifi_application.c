...
//!
//! This example defaults to using BoosterPack1. If you would like to use
//! BoosterPack2 instead please change the define CC3000_USE_BOOSTERPACK1 in
//! the project settings to CC3000_USE_BOOSTERPACK2 and rebuild.  When using
//! the BoosterPack2 connector, make sure that jumpers J16 and J17 are both
//! set to the SPI position.
//!
...
//
//*****************************************************************************
volatile uint32_t g_ui32SmartConfigFinished, g_ui32CC3000Connected = 0,
                  g_ui32CC3000DHCP = 0,g_ui32OkToDoShutDown,
...
initDriver(void)
{
    ...
    //
    // Turn on the red LED to indicate that we are active and initiated
    // WLAN successfully.
    //
    turnLedOn(RED_LED);
...
ScreenInit(void)
{
    ...
    //
    FrameDraw(&g_sContext, "cc3000-basic-wifi-application");
    ...
    GrStringDrawCentered(&g_sContext,
                         "This example is driven using a command line",
                         -1, 160, 80, false);
    GrStringDrawCentered(&g_sContext,
                         "provided by UART0.  Use a terminal program on",
                         -1, 160, 100, false);
    GrStringDrawCentered(&g_sContext,
                         "your host system with serial parameters set to",
                         -1, 160, 120, false);
    GrStringDrawCentered(&g_sContext,
                         "115200bps, 8-N-1.",
                         -1, 160, 140, false);
...
void StartSmartConfig(void)
{
    ...
    //
    MAP_SysCtlDelay(g_ui32SysClock / 3000);
...
CMD_help(int argc, char **argv)
{
    ...
    while(g_psCmdTable[i32Index].pcCmd)
    {
        ...
        //
        // Wait for the UART transmit buffer to clear before we add more text.
        //
        UARTFlushTx(false);
    }
...
CMD_receiveData(int argc, char **argv)
{
    ...
    else if(g_ui32SocketType == IPPROTO_TCP)
    {
        ...
        do
        {
            ...
            {
                //
                // Add columnn wrapping to make output pretty.
                //
                if( ((ui32Count % 60) == 0) && (ui32Count > 0))
                {
                    UARTprintf("\n    ");
...
CMD_bind(int argc, char **argv)
{
    ...
    //
    // Call CC3000 to bind the socket.
    //
...
// This configuration can also be accomplished by running the smartconfig app.
...
CMD_ipConfig(int argc, char **argv)
{
    ...
    else
    {
        ...
        // Handle IP address.
        //
        i32Check = DotDecimalDecoder(argv[1], &ui8IP[0],
            &ui8IP[1],&ui8IP[2], &ui8IP[3]);
        ...
        if(argc == 4)
        {
            ...
            //
            i32Check = DotDecimalDecoder(argv[3],
                                        &ui8NetMask[0], &ui8NetMask[1],
                                        &ui8NetMask[2], &ui8NetMask[3]);
    ...
    //
    // Tell the CC3000 firmware to configure the IP address.
    //
...
main(void)
{
    ...
    while(1)
    {
        ...
        //
        // A '\r' was detected so get the line of text from the buffer.
        //
